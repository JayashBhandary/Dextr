...
class SqlToken {
  const SqlToken(this.text, this.token);
...
List<SqlToken> tokenizeSql(String source) {
  ...
  void flushPlain() {
    ...
    tokens.add(SqlToken(buffer.toString(), null));
    buffer.clear();
  }

  void emit(String text, AstryxSyntaxToken token) {
    flushPlain();
    tokens.add(SqlToken(text, token));
  }
  ...
  while (i < source.length) {
    ...
    if (rest.startsWith('--') || rest.startsWith('#')) {
      ...
      emit(source.substring(i, stop), AstryxSyntaxToken.comment);
      i = stop;
      continue;
    }
    ...
    if (char == "'") {
      ...
      emit(source.substring(i, j), AstryxSyntaxToken.string);
      i = j;
      continue;
    }
    ...
    if (char == '"' || char == '`') {
      ...
      emit(source.substring(i, stop), AstryxSyntaxToken.property);
      i = stop;
      continue;
    }
    ...
    if (_isDigit(char)) {
      ...
      emit(source.substring(i, j), AstryxSyntaxToken.number);
      i = j;
      continue;
    }
    ...
    if ((char == ':' || char == r'$' || char == '@') &&
      ...
      emit(source.substring(i, j), AstryxSyntaxToken.variable);
      i = j;
      continue;
    }
    ...
    if (_operatorChars.contains(char)) {
      emit(char, AstryxSyntaxToken.operator);
      i++;
      continue;
    }

    if (_punctuationChars.contains(char)) {
      emit(char, AstryxSyntaxToken.punctuation);
      i++;
      continue;
    }
